...
from dataset.stc import STCDataset, STC_CLASS
...
def parse_args():
    ...
    parser.add_argument("--data_root", type=str, default="D:/Dataset/ShanghaiTech/bin/")
    parser.add_argument("--resize", type=int, default=256)
    parser.add_argument("--crop_size", type=int, default=256)

    parser.add_argument("--k", type=int, default=30)
    parser.add_argument("--gpu_id", type=str, default="0")
    parser.add_argument("--save_path", type=str, default='result_stc')
...
def main():
    ...
    args.save_path = os.path.join(args.save_path, 'dpfc_kmeans_covariance', f"{args.backbone}", f"k_{args.k}")
    ...
    for scene_class in STC_CLASS:

        trainset = STCDataset(root_path=args.data_root, is_train=True, scene_class=scene_class, resize=args.resize,
                                cropsize=args.crop_size)
        trainloader = DataLoader(trainset, batch_size=args.img_batch, shuffle=False, pin_memory=False)
        testset = STCDataset(root_path=args.data_root, is_train=False, scene_class=scene_class, resize=args.resize,
                               cropsize=args.crop_size)
        ...
        # 1. 提取训练集的特征
        train_feas_pkl = os.path.join(train_feas_path, f"train_{args.backbone}_{scene_class}.pkl")
        # train_feas_pkl = os.path.join(temp_path, f"train_{args.backbone}_{class_name}.pkl")
        if not os.path.exists(train_feas_pkl):
            for x, _ in tqdm(trainloader, desc=f"[{scene_class} train feature extract]"):
        ...
        # testing
        test_criterion_pkl = os.path.join(temp_path, f"test_{args.backbone}_{scene_class}_criterion.pkl")
        if not os.path.exists(test_criterion_pkl):
            torch.cuda.empty_cache()

            # Kmeans 聚类
            # center_nums = {'layer1': 3, 'layer2': 3, 'layer3': 3}
            kmeans_pkl = os.path.join(temp_path, f"test_{args.backbone}_{scene_class}_gmm.pkl")
            if not os.path.exists(kmeans_pkl):
                kmeans_ = {}
                for k, v in train_feas.items():
                    logger.info(f"Model GMM {scene_class} feature {k}...")
                    v = v.transpose(1, 3).flatten(0, 2)
                    # cpu
                    _means_, _vars_ = _kmeans_fun_gpu(v, K=center_nums[k])
            ...
            test_mask_list = []
            # 2. 提取测试集的特征
            for x, mask in tqdm(testloader, desc=f"[{scene_class} test feature extract]"):
                torch.cuda.empty_cache()
            ...
            score_map_list = []
            for idx in tqdm(range(test_feas["layer1"].size(0)), desc=f"[{scene_class} test pixel anomaly segment]"):
                torch.cuda.empty_cache()
                score_maps = []
            ...
            visualize(test_img_list, test_mask_list, score_map_list, threshold, class_name=scene_class, save_path=args.save_path, num=1000)
        ...
        logger.info('%s pixel ROCAUC: %.3f' % (scene_class, pixel_level_ROCAUC))
        logger.info(f"\n")
        ax.plot(fpr, tpr, label='%s ROCAUC: %.3f' % (scene_class, pixel_level_ROCAUC))
